		...
		<div class="feature-content">
			...
			<div class="feature-header">
				...
				<h4 class="feature-title heading-title entry-title">
					<a target="<?php echo esc_attr($target); ?>" href="<?php echo ($link != '')?esc_url($link):'javascript: void(0)' ?>"><?php echo wp_kses($title, array('br' => array())) ?></a>
				</h4>
				<?php endif; ?>
				
				<?php if( strlen($sub_title) > 0 ): ?>
				<div class="feature-subtitle">
					<?php echo wp_kses($sub_title, array('br' => array())) ?>
				</div>
				<?php endif; ?>
			
				<?php if( strlen($excerpt) > 0 ): ?>
				<div class="feature-excerpt">
					<?php echo wp_kses($excerpt, array('br' => array())) ?>
				</div>
				<?php endif; ?>
...
function ts_price_table_shortcode( $atts ){
	...
	if( $style == 'style-1'){
		...
		$inline_style .= $selector.' .button-price-table{background: '.$color_scheme.'; color:#ffffff; border-color:'.$color_scheme.';}';
		$inline_style .= $selector.' .button-price-table:hover{background: transparent; color:'.$color_scheme.'; border-color:'.$color_scheme.';}';
	}
	if($style == 'style-2'){
		$inline_style .= $selector.' header{background:'.$color_scheme.';}';
		$inline_style .= $selector.'.style-2 .button-price-table{color: '.$color_scheme.';}';
		if($active_table){
...
function ts_testimonial_shortcode($atts){
	extract(shortcode_atts(array(
						...
						,'columns'				=> 1
						,'per_page'				=> 4
						,'text_color_style'		=> 'quote-default'
						,'ids'					=> ''
						,'show_avatar'			=> 0
						,'show_name'			=> 1
						,'show_byline'			=> 0
		...
		<div class="ts-testimonial-wrapper ts-shortcode <?php echo esc_attr(implode(' ', $classes)); ?>" <?php echo implode(' ', $data_attr); ?>>
			...
			<header class="shortcode-heading-wrapper">
				<h2 class="shortcode-title">
					<?php 
						if($quote_icon){
							echo wp_get_attachment_image($quote_icon, 'thumbnail', 0, array('class'=>''));
						}
						echo esc_html($title);
					?>
				</h2>
			...
			while( $testimonials->have_posts() ){
				...
				if( function_exists('mymedi_the_excerpt_max_words') ){
					$content = mymedi_the_excerpt_max_words($excerpt_words, $post, false, '', false);
				}
				else{
							...
							<figure>
								<?php if( has_post_thumbnail() ): ?>
									<a href="<?php echo esc_url($link); ?>">
										<?php the_post_thumbnail('ts_portfolio_thumb'); ?>
									</a>
								<?php endif; ?>	
...
function ts_banner_shortcode( $atts ){
	extract(shortcode_atts(array(
						...
						,'bg_color'								=> '#ffffff'
						,'heading_title'						=> ''
						,'heading_text_color'					=> '#103178'
						,'discount'								=> ''
						,'discount_color'						=> '#ff9923'
						,'show_button'							=> 0
						,'button_text'							=> 'Shop Now'
						,'button_background_color'				=> '#103178'
	...
	$classes[] = $image_radius?'banner-radius':'';
	...
	if( $button_background_color && $button_text_color ){
		$style .= $selector.' .banner-wrapper .ts-banner-button a{border-color:'. $button_background_color .'; background-color:'. $button_background_color .'; color:'. $button_text_color .'}';
		$style .= $selector.' .banner-wrapper .ts-banner-button a:hover{border-color:'. $button_background_color .'; background-color: transparent; color:'. $button_background_color .'}';
	}
	if( $bg_color ){
...
function ts_button_shortcode($atts, $content=null){
	extract(shortcode_atts(array(	
					...
					,'bg_color'				=> '#ffffff'
					,'bg_color_hover'		=> '#ff9923'
					,'border_color'			=> '#ff9923'
					,'border_color_hover'	=> '#ff9923'
					,'border_width'			=> '1'
					,'border_radius'		=> ''
					,'add_icon'				=> 0
					,'font_icon'			=> ''
...
if( !function_exists('ts_mailchimp_subscription_shortcode') ){
	function ts_mailchimp_subscription_shortcode( $atts ){
		...
		$args = array(
			'before_widget' => '<' . ( $title?'section':'div' ) . ' class="widget-container %s">'
			,'after_widget' => '</' . ( $title?'section':'div' ) . '>'
...
if( !function_exists('ts_social_shortcode') ){
	function ts_social_shortcode( $atts ){
		...
		$instance = compact('title', 'facebook_url', 'twitter_url', 'linkedin_url', 'flickr_url', 'vimeo_url', 'youtube_url', 'viber_number', 'skype_username', 'instagram_url', 'pinterest_url', 'custom_link', 'custom_text', 'custom_font', 'show_tooltip');
	...
	function ts_blogs_shortcode( $atts, $content = null){
		...
		if( $posts->have_posts() ):
			...
			$atts = compact('layout', 'columns', 'categories', 'per_page', 'orderby', 'order'
							,'item_layout', 'show_title', 'show_thumbnail', 'show_author', 'show_tags', 'show_categories'
...
if( !function_exists('ts_get_blog_items_content_shortcode') ){
	function ts_get_blog_items_content_shortcode($atts, $posts = null){
		...
		$blog_thumb_size = 'mymedi_blog_thumb_shortcode';
		if( $columns == 1 ){
			$blog_thumb_size = 'mymedi_blog_thumb';
		}
		...
		if( $posts->have_posts() ):
			...
			while( $posts->have_posts() ): $posts->the_post();
				...
				$item_extra_class = ($key % $columns == 0)?'first':(($key % $columns == $columns - 1)?'last':'');
				?>
				<article class="item <?php echo esc_attr($post_format); ?> <?php echo !$show_thumbnail?'no-featured-image':'' ?> <?php echo esc_attr($item_class.' '.$item_extra_class) ?>">
					<?php
					...
					<?php if( $post_format != 'quote' ): ?>
						...
						<div class="entry-content">
							...
							<?php if( $show_excerpt && function_exists('mymedi_the_excerpt_max_words') ): ?>
							<div class="excerpt"><?php mymedi_the_excerpt_max_words($excerpt_words, '', true, '', true); ?></div>
							<?php endif; ?>
							
							<?php if( $show_readmore ): ?>
							<div class="entry-meta-bottom">
								<!-- Blog Read More Button -->
								<a class="button-readmore small" href="<?php the_permalink() ; ?>"><?php esc_html_e('Read More', 'themesky'); ?></a>
...
if( !function_exists('ts_google_map_shortcode') ){
	function ts_google_map_shortcode($atts, $content = ''){
		...
		$gmap_api_key = function_exists('mymedi_get_theme_options')?mymedi_get_theme_options('ts_gmap_api_key'):'';
