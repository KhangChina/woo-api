...
class TS_Product_Filter_By_Color_Widget extends WP_Widget{
	...
	function widget( $args, $instance ) {
		...
		$show_product_count = isset($instance['show_product_count'])?$instance['show_product_count']:0;
		...
		if ( count($terms) > 0 ) {
			...
			foreach ( $terms as $term ) {
				...
				if( absint($datas['ts_color_image']) > 0  ){
					...
					echo '<span class="color-name">'.$term->name.'</span>';
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['show_product_count'] =  $new_instance['show_product_count'];	
		return $instance;
	}
	...
	?>
		...
		<p>
			<input type="checkbox" value="1" id="<?php echo $this->get_field_id('show_product_count'); ?>" name="<?php echo $this->get_field_name('show_product_count'); ?>" <?php checked($show_product_count, 1); ?> />
			<label for="<?php echo $this->get_field_id('show_product_count'); ?>"><?php esc_html_e('Show product count', 'themesky'); ?></label>
		</p>
		<?php 
	}
